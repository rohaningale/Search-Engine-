...
public class SearchEngineTester {
	...
	public static void searchKeyword(String keyword) throws Exception {
        ...
        // loop through the document IDs in the row.
        ...
        for (KeyValue kv : indexRow.list()) {
            ...
            //get the document ID of one page, as well as the keyword's frequency in that page
	        pageDocId = Bytes.toString(kv.getQualifier());
			freq = Bytes.toInt(kv.getValue());
			
            // get the URI of the page from clueWeb09DataTable
            ...
            // get the page rank value of this page from clueWeb09PageRankTable
	        Get gPageRank = new Get(Bytes.toBytes(pageDocId));
	        Result pageRankRes = prTable.get(gPageRank);
	        pageRank = Bytes.toFloat(pageRankRes.list().get(0).getQualifier());
		    // END of your code
